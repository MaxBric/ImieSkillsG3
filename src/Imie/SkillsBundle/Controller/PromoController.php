...
class PromoController extends Controller {
  ...
  if($form->isValid()){
    try{
      ...
      $em->persist($promo);
      $em->flush();
      $req->getSession()->getFlashBag()->add('success', 'Promo ajoutée');
...
public function detailsAction(Request $req, $id) {
  ...
  return $this->render('ImieSkillsBundle:Promo:details.html.twig',array(
    'users'=>$users
  ));
}
...
public function modifyAction(Request $req, $id) {
  ...
  if ($form->isValid()) {
    try {
      $em->flush();
      $req->getSession()->getFlashBag()->add('success', 'Promo modifiée');
...
public function deleteAction(Request $req, $id) {
  ...
  try {
    $em->remove($promo);
    $em->flush();
    $req->getSession()->getFlashBag()->add('success', 'Promo supprimée');
  } catch (\Doctrine\DBAL\DBALException $e) {
